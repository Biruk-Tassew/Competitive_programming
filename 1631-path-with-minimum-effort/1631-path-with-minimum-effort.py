        ...
        while heap:
            ...
            for i in directions:
                ...
                if 0 <= n_r < len(heights) and 0 <= n_c < len(heights[0]):
                    max_effort = max(cur_effort, abs(heights[row][col]-heights[n_r][n_c]))
                    if all_efforts[n_r][n_c] > max_effort:
                        
                        all_efforts[n_r][n_c] = max_effort
                        heapq.heappush(heap, (max_effort, n_r, n_c))